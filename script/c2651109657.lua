...
function s.costfilter(c)
	return c:IsAttribute(ATTRIBUTE_DARK) and c:GetBaseAttack()==0
end
...
function s.filter(c)
	return c:IsFaceup() and (c:IsLinkMonster() or c:IsPendulumMonster() or c:IsXyzMonster()) and (c:GetAttack()>0 or not c:IsDisabled())
end
...
function s.con(e,tp,eg,ep,ev,re,r,rp)
	return eg:GetFirst():IsSummonType(SUMMON_TYPE_LINK+SUMMON_TYPE_PENDULUM+SUMMON_TYPE_XYZ)
end
