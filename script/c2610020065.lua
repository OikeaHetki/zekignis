...
function s.initial_effect(c)
	...
	e0:SetRange(LOCATION_MZONE+LOCATION_GRAVE+LOCATION_REMOVED+LOCATION_HAND)
	e0:SetValue(33396948)
	c:RegisterEffect(e0)
	--special summon
...
function s.spfilter(c)
	return c:GetLevel()==5 and c:IsType(TYPE_NORMAL)
end
...
function s.costfilter(c)
	return c:IsType(TYPE_MONSTER+TYPE_NORMAL) and c:IsDiscardable()
end
...
function s.filter(c)
	return c:IsType(TYPE_MONSTER+TYPE_NORMAL) and c:IsAbleToGrave()
end
