...
--zekpro version (cannot summon non-prophecy; tracks only Spellbooks)
local s,id=GetID()
...
function s.sfilter(c)
	return c:IsSetCard(SET_SPELLBOOK) and c:GetCode()~=id and c:IsSpell() and c:IsAbleToHand()
end
function s.spfilter(c,lv,e,tp)
	return c:IsLevelBelow(lv) and c:IsSetCard(SET_PROPHECY) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
