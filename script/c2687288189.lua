...
function s.initial_effect(c)
	...
	e3:SetCategory(CATEGORY_DESTROY+CATEGORY_REMOVE+CATEGORY_DAMAGE)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,nil,tp,LOCATION_MZONE,LOCATION_MZONE,1,ct,nil)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,#g,0,0)
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,1000)
	Duel.SetPossibleOperationInfo(0,CATEGORY_REMOVE,nil,1,g:GetFirst():GetControler(),LOCATION_HDEG)
end
	...
	if #g>0 and Duel.Destroy(g,REASON_EFFECT)~=0 then
		...
		for tc in aux.Next(og) do
			if tc:IsAttribute(ATTRIBUTE_DARK) then
				local sg=Duel.GetMatchingGroup(s.rmfilter,tc:GetControler(),LOCATION_HDEG,0,nil,tc:GetCode())
				rg:Merge(sg)
			end
		end
		if #rg>0 then
