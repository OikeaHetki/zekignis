--炎塵爆発
--Dustflame Blast
--zekpro version (targets; banishes any number as cost)
local s,id=GetID()
...
function s.costfilter(c)
	return c:IsSetCard(SET_LAVAL) and c:IsMonster() and c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true)
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		if e:GetLabel()==1 then
			e:SetLabel(0)
			return aux.SelectUnselectGroup(rg,e,tp,nil,99,s.rescon,0)
		else return false end
	end
	e:SetLabel(0)
	local rsg=aux.SelectUnselectGroup(rg,e,tp,nil,99,s.rescon,1,tp,HINTMSG_REMOVE,s.rescon)
