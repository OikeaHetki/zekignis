...
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(Card.IsFaceup,tp,LOCATION_MZONE+LOCATION_GRAVE,0,nil)
	return #g>0 and g:FilterCount(Card.IsType,nil,TYPE_TOON)==#g
	and eg:IsExists(Card.IsPreviousLocation,1,nil,LOCATION_EXTRA)
end
function s.filter(c,tp)
	return c:IsCode(15259703) and c:GetActivateEffect():IsActivatable(tp,true,true)
end
