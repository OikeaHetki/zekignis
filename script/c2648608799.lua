...
function s.thfilter(c)
	local lv=c:GetLevel()
	return c:IsFaceup() and lv==1 and c:IsType(TYPE_PENDULUM) and c:IsAbleToHand()
end
...
--tg
function s.filter(c)
	return c:IsRace(RACE_WINGEDBEAST) and c:GetLevel()==1 and c:IsAbleToGrave()
end
